...
public class ClientManagerController extends CommonController{
    @Autowired
    private LocalRepo localRepo;
    
    @RequestMapping("/index")
    public String objectIndex(Model model){
        model.addAttribute("PAGE_TITLE", "Client Config | appSync");        
        return "client_manager_index";
    }
    ...
    public @ResponseBody Response getObjectInfo(@RequestBody Map<String, String> request) {
        try{
            return Response.OK(localRepo.getClientConfig());
        } catch(Exception ex){
            ex.printStackTrace();
    ...
    public @ResponseBody Response manageObjectInfo(@PathVariable("type") String type, @RequestBody Map<String, Object> request) {
        try{
            return Response.OK(localRepo.manageClientConfig(request, type));
        } catch(Exception ex){
            ex.printStackTrace();
            return Response.ERROR(ex.getMessage());
        }
